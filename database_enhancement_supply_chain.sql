-- SUPPLY CHAIN ENHANCEMENT FOR EXISTING DATABASE
...
ADD COLUMN IF NOT EXISTS surveyor_level text CHECK (surveyor_level IN ('trainee', 'qualified', 'senior', 'principal')),
...
ALTER TABLE public.user_profiles 
ADD COLUMN IF NOT EXISTS customer_type text CHECK (customer_type IN ('individual', 'business', 'trust', 'estate')),
ADD COLUMN IF NOT EXISTS business_name text,
ADD COLUMN IF NOT EXISTS customer_since date DEFAULT CURRENT_DATE,
ADD COLUMN IF NOT EXISTS risk_category text DEFAULT 'standard' CHECK (risk_category IN ('low', 'standard', 'elevated', 'high')),
...
-- Add supply chain specific permissions to existing role_permissions table
INSERT INTO public.role_permissions (role, permission_category, permission, description, requires_additional_auth)
VALUES
-- Contractor permissions
('contractor', 'projects', 'view_assigned_projects', 'View projects they are assigned to', false),
('contractor', 'projects', 'update_project_progress', 'Update progress on assigned projects', false),
('contractor', 'quotes', 'submit_quotes', 'Submit quotes for project work', false),
('contractor', 'documents', 'upload_progress_photos', 'Upload work progress photographs', false),
('contractor', 'invoices', 'submit_invoices', 'Submit invoices for completed work', false),

-- Contractor admin permissions (for contractor company admins)
('contractor_admin', 'team', 'manage_team_members', 'Manage contractor team assignments', false),
('contractor_admin', 'quotes', 'approve_quotes', 'Approve quotes before submission', false),
('contractor_admin', 'projects', 'view_all_company_projects', 'View all projects assigned to company', false),

-- Surveyor permissions
('surveyor', 'surveys', 'conduct_surveys', 'Conduct property surveys', false),
('surveyor', 'surveys', 'approve_survey_reports', 'Approve and submit survey reports', false),
('surveyor', 'reserves', 'set_initial_reserves', 'Set initial damage reserve amounts', false),
('surveyor', 'damage_items', 'create_damage_assessments', 'Create detailed damage assessments', false),

-- Enhanced admin permissions for supply chain management
('admin', 'contractors', 'manage_all_contractors', 'Full contractor management access', false),
('admin', 'surveyors', 'manage_all_surveyors', 'Full surveyor management access', false),
('admin', 'performance', 'view_all_performance_data', 'View performance data for all users', false),
('admin', 'supply_chain', 'configure_supply_chain', 'Configure supply chain settings', true)

ON CONFLICT (role, permission_category, permission) DO NOTHING;
...
-- 3. Check new supply chain permissions
-- SELECT * FROM public.role_permissions WHERE role IN ('contractor', 'contractor_admin', 'surveyor');
